...
export class AiService implements OnModuleInit {
  ...
  async processMessage(userId: string, userMessage: string) {
    try {
      ...
      // Capturar cuando se agoten los tokens / límite de cuota del proveedor (HTTP 429)
      if (
        error?.status === 429 ||
        error?.code === 429 ||
        ...
        throw new HttpException(
          {
            ...
            message:
              'Cheems IA se quedó sin tokens temporales de Gemini uwu. Intenta de nuevo en un par de minutos.',
          },
          HttpStatus.TOO_MANY_REQUESTS,
        );
      }

      // Si es otro tipo de error, relanzarlo para que NestJS lo maneje normalmente
      throw error;
    }
  }
}